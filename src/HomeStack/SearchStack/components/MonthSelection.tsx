...
import { PreviousReports, PreviousReportsData, Report } from '../../../shared/types';
...
export const MonthSelection: React.FC<MonthSelectionProps> = ({ item, report, year }) => {
  const [expanded, setExpanded] = useState<boolean>(false);
  const [previousReportData, setPreviousReportData] = React.useState<PreviousReportsData | null>(null);
    ...
    return (
      ...
      >
        <ListItem.Content dataDetectorType={undefined}>
          <ListItem.Title dataDetectorType={undefined}>
            {`Date: ${item.report_date}`}
          </ListItem.Title>
          <ListItem.Subtitle dataDetectorType={undefined} style={{ fontWeight: 'bold', color: 'green' }}>{item.file_extension}</ListItem.Subtitle>
        </ListItem.Content>
        <ListItem.Chevron dataDetectorType={undefined} />
      </ListItem>
    )
  }
  ...
  return (
    <ListItem.Accordion
      ...
      content={
        <>
          <Icon name="calendar" type="entypo" color="#b34836" style={{ paddingRight: 20 }} />
          <ListItem.Content dataDetectorType={undefined}>
            <ListItem.Title dataDetectorType={undefined}>{monthNames[Number(item.item) - 1]}</ListItem.Title>
          </ListItem.Content>
        </>
      }
    >
